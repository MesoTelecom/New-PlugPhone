...
const { send, sendImage, sendVideo, sendAudio, sendDocument, sendTemplate, download, test, getImage, getAudio, getDocument, reciveMediaLink, geraMedia } = require('/meso/whatsapp/webhook/methods.js');
...
class ExpressController {
  ...
  initializeRoutes() {
    ...
    this.expressAppWrapper.post('/upload-document', uploadDocument.single('document'), async (req, res) => {
      if (!req.file) {
...
this.expressAppWrapper.post("/geraDocument", async (req, res, next) => {
  ...
  try {
    ...
    this.expressAppWrapper.get("/buscarcontatos/:tipo/:usuario", async (req, res, next) => {
      ...
      qry = `select * from meso_contatos where usuario = '${usuario}' or usuario is NULL and setor = '${tipo}'`
    ...
    */
    ...
    this.expressAppWrapper.post("/senddocument", async (req, res) => {
      ...
      sendDocument(to, id, usuario, res)
    })

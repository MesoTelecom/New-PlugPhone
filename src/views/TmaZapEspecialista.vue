<template>
    <div class="limiter">
        ...
        <v-card class="filtrorelatorios">
            ...
            <v-select :items="items" label="Agentes" v-model="especialista" class="filtro"></v-select>
...
export default {
    ...
    data() {
        return {
            ...
            headers: [
                ...
                { text: "Quant Ligação", value: "quantidade" },
                { text: "Tempo em Ligação", value: "tempo" },
    ...
    methods: {
        tmazap: async function () {
            let tudo = await api.get(`/tmazapespecialista/${this.especialista}/${this.d1}/${this.d2}`)
        ...
        listarAnalista: async function () {
            let listaAnalista = await api.get(`/listaespecialista`);
            console.log(listaAnalista);
            let listaAnalistaTotal = listaAnalista.data.dados;
            let especialistas = [];
            listaAnalistaTotal.forEach((d) => {
                especialistas = d.usuario;
                this.items.push([especialistas]);
            });
        },
        saveCSV() {
            this.filename = "PlugPhone TMA";
            //================================================================
            let csvFile = `"Nome do Agente";"Plataforma";"Quant Ligação";"Tempo em Ligação";"Media Tempo";"Maxima Duração"\n`;
            this.dados.forEach((a) => {
                csvFile += `"${a.nomeAgente}";"${a.plataforma}";"${a.quantidade}";"${a.tempo}";"${a.mediaTempo}";"${a.maximaDuracao}"\n`;
            });

  ...
  export default {
    ...
    data() {
      return {
        search: "",
        headers: [
         { text: "Mensagens Recebidas", value: "sucesso" },
          { text: "Mensagens Respondidas", value: "abandon" },
          { text: "Mensagens Não Respondidas", value: "espera" },
          { text: "Tempo Respondidas", value: "tempoRespondida" },
          { text: "Tempo Não Respondidas", value: "tempoNaoRespondida" },
        ],
        dados: [
            {
                sucesso: 16,
                abandon: 11,
                espera: 5,
                tempoRespondida: "03:42:51",
                tempoNaoRespondida: "02:57:33"
            }
        ],
        listafila: [],
        fila: "",
    ...
    methods: {
      ...
      exibir: async function () {
        ...
        let a = await api.get(
          `/recebidafila/${this.fila}/${this.d1}/${this.d2}`
        );
  
        let recebida = a.data.dados.length;
  
        let b = await api.get(
          `/recebidasucesso/${this.fila}/${this.d1}/${this.d2}`
        );
  
        let sucesso = b.data.dados.length;
  
        let c = await api.get(
          `/recebidaabandon/${this.fila}/${this.d1}/${this.d2}`
        );
  
        let abandon = c.data.dados.length;
  
        let d = await api.get(
          `/recebidajoin/${this.fila}/${this.d1}/${this.d2}`
        );
  
        let join = d.data.dados.length;
  
        let e = await api.get(
          `/recebidaespera/${this.fila}/${this.d1}/${this.d2}`
        );
  
        let tempo = e.data.dados;
  
        let h = await api.get(
          `/recebidaesperaatende/${this.fila}/${this.d1}/${this.d2}`
        );
      ...
      saveCSV() {
        ...
        this.filename = "PlugPhone Filas";
        //================================================================
         let csvFile = `"Filas";"Sucesso";"Abandonado";"Tempo de Espera";"Tempo de espera Chamadas Atendidas";"Total de chamadas"\n`;

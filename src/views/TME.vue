...
export default {
  ...
  methods: {
    ...
    exibir: async function () {
      // TOTAL DE ESPERA
      let somatme = await api.get(`/somaespera/${this.fila}/${this.d1}/${this.d2}`);
      let somaminutos = 0;
      if (somatme.data.dados.length) {
        somaminutos = (somatme.data.dados[0].somaespera || 0) / 60;
      }

      // MÉDIA
      let mediatme = await api.get(`/mediaespera/${this.fila}/${this.d1}/${this.d2}`);
      let mediaminutos = 0;
      if (mediatme.data.dados.length) {
        mediaminutos = (mediatme.data.dados[0].mediaespera || 0) / 60;
      }

      // MÍNIMO
      let minimotme = await api.get(`/minimoespera/${this.fila}/${this.d1}/${this.d2}`);
      let minimominutos = 0;
      if (minimotme.data.dados.length) {
        minimominutos = (minimotme.data.dados[0].minimaespera || 0) / 60;
      }

      // MÁXIMO
      let maximotme = await api.get(`/maximoespera/${this.fila}/${this.d1}/${this.d2}`);
      let maximominutos = 0;
      if (maximotme.data.dados.length) {
        maximominutos = (maximotme.data.dados[0].maximaespera || 0) / 60;
      }

      // TOTAL DE CHAMADAS
      let totalchamadas = await api.get(`/totalchamadasespera/${this.fila}/${this.d1}/${this.d2}`);
